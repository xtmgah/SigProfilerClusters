...
####### Most of this part has been upadted !!!
def pullVaf (project, project_path, sanger=True, TCGA=False, standardVC=False, correction=True):
			...
			with open(vcf_path + vcfFile) as f:
				for lines in f:
					...
					if len(lines) > 8:
						Arr = lines[8].split(":")
						if "VAF" in Arr:
							Input_Type = "non-Sanger-VAF"
						elif "AF" in Arr:
							Input_Type = "non-Sanger-AF"
						elif "AD" in Arr:
							Input_Type = "non-Sanger-AD"
					...
					###### 001 if "," is detected, return -2.5
					if re.search(r',', ref):
						vaf = -2.5
					elif re.search(r',',alt):
						vaf = -2.5
					
					###### 002 if 8 columns for Sange:
					elif Input_Type == "Sanger":
						INFO = lines[7]
						INFO_arr = INFO.split(";")
						for ii in INFO_arr:
							if re.search(r'VAF=',ii):
								vaf = ii.split("=")[1]
							elif re.search(r'AF=',ii):
								vaf = ii.split("=")[1]

					###### 003 if >8 columns for non-Sange:
					elif Input_Type == "non-Sanger-VAF":
						FORMAT = lines[8]
						FORMAT_arr = FORMAT.split(":")
						#print(FORMAT_arr)
						AF_Index = FORMAT_arr.index("AF")
						AF_Arr = lines[9].split(":")
						vaf = AF_Arr[AF_Index]
						if vaf == ".":
							vaf = -1.5

						Note="\n\nExtracting VAF value...\n\n"
						#print(Note)
						#print(lines)
						#print(vaf)

					elif Input_Type == "non-Sanger-AF":
						FORMAT = lines[8]
						FORMAT_arr = FORMAT.split(":")
						#print(FORMAT_arr)
						AF_Index = FORMAT_arr.index("AF")
					...
					elif Input_Type == "non-Sanger-AD":
						FORMAT = lines[8]
						FORMAT_arr = FORMAT.split(":")
						#print(FORMAT_arr)
						AD_Index = FORMAT_arr.index("AD")
						
						if AD_Index == 999:
							pass
						else:
							AD_Arr = lines[9].split(":")
							AD = AD_Arr[AD_Index]
							dd = AD.split(",")
							#print(len(dd))
							if len(dd) == 2:
	...
	####### Find input and ouput:
	print(clusteredMutsFile)
	print(clusteredMutsPath + project + "_clustered_vaf.txt")


	non_clusteredMuts_Path = project_path + "output/vcf_files" + path_suffix + "/" + project + "_nonClustered" + "/SNV/" + project + "_nonClustered.txt"
	non_clusteredMuts_Path_Vaf = project_path + "output/vcf_files" + path_suffix + "/" + project + "_nonClustered" + "/SNV/" + project + "_nonClustered_Vaf.txt"
	...
	with open(non_clusteredMuts_Path) as f, open(non_clusteredMuts_Path_Vaf, "w") as out:
		next(f)
		# print("HEADER", file=out)
		print("\t".join([x for x in ["project", "samples","ID","genome","mutType","chr","start","end", "ref", "alt", "mutClass", "IMDplot", "IMD", "VAF/CCF"]]), file=out)
		for lines in f:
